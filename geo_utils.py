"""
M贸dulo de Geolocalizaci贸n para el Sistema de Logging

Este m贸dulo proporciona funcionalidades para obtener informaci贸n geogr谩fica
del usuario basada en su direcci贸n IP.

Caracter铆sticas:
- M煤ltiples APIs de respaldo (ipapi.co, ip-api.com, ipinfo.io)
- Cache de resultados para evitar llamadas repetidas
- Manejo robusto de errores
- Timeout configurable
- Detecci贸n de IP p煤blica
"""
...
class GeoLocator:
    """
    Clase para obtener informaci贸n geogr谩fica basada en IP.
    """
    ...
    def __init__(
        ...
        Args:
            cache_duration_minutes: Duraci贸n del cache en minutos
    ...
    def _get_public_ip(self) -> Optional[str]:
        """
        Obtiene la direcci贸n IP p煤blica del usuario.
        
        Returns:
            IP p煤blica o None si no se puede obtener
        """
        ...
        # Si todo falla, intentar obtener IP local (no ser谩 p煤blica)
        try:
    ...
    def _get_location_from_ipinfo_io(self, ip: str) -> Optional[Dict]:
        """
        Obtiene ubicaci贸n usando ipinfo.io (gratuito, 50k req/mes).
    ...
    def get_location(self, ip: Optional[str] = None) -> Dict:
        """
        Obtiene la informaci贸n geogr谩fica del usuario.
        
        Args:
            ip: IP espec铆fica a consultar. Si None, obtiene la IP p煤blica actual.
        
        Returns:
            Diccionario con informaci贸n geogr谩fica
        """
        # Si no se proporciona IP, obtener la p煤blica
        if ip is None:
        ...
        # Si no se pudo obtener, retornar datos b谩sicos
        if not location_data:
            location_data = {
                ...
                "timezone": "N/A",
                "org": "N/A",
                "error": "No se pudo obtener informaci贸n geogr谩fica"
            }
    ...
    def get_location_by_hostname(self, hostname: str) -> Dict:
        """
        Obtiene la ubicaci贸n basada en un hostname.
    ...
    def is_local_ip(self, ip: str) -> bool:
        ...
        Returns:
            True si es IP local, False si es p煤blica
        """
        if not ip:
            return False
    ...
    def clear_cache(self):
        """Limpia el cache de geolocalizaci贸n."""
        self.cache = {}
...
def get_current_location() -> Dict:
    """
    Obtiene la ubicaci贸n geogr谩fica actual de forma r谩pida.
...
if __name__ == "__main__":
    ...
    print("\n3. Probando con IP espec铆fica (8.8.8.8 - Google DNS):")
    google_location = locator.get_location("8.8.8.8")
    print(json.dumps(google_location, indent=2, ensure_ascii=False))
    
    print("\n4. Probando detecci贸n de IP local:")
    print(f"驴127.0.0.1 es local? {locator.is_local_ip('127.0.0.1')}")
    print(f"驴8.8.8.8 es local? {locator.is_local_ip('8.8.8.8')}")
