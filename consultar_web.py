...
def load_resources():
    ...
    with st.spinner('Inicializando LLM y embeddings (de forma perezosa)...'):
        ...
        try:
            ...
            # Auto-construir índice FAISS si no existe
            if not os.path.exists("faiss_index"):
                st.info("🔨 Primera ejecución: construyendo índice FAISS. Esto tomará 2-3 minutos...")
                from auto_build_index import build_faiss_index
                success = build_faiss_index(api_key)
                if not success:
                    st.error("❌ No se pudo construir el índice FAISS automáticamente.")
                    st.stop()
...
with st.sidebar:
    ...
    with st.expander("â“ **CÃ³mo Hacer Preguntas**", expanded=True):
        ...
        **4ï¸âƒ£ Por NÃºmero**
        ...
        ðŸ“š **[Ver GuÃ­a Completa](https://github.com/arguellosolanogerardo-cloud/consultor-gerard/blob/main/GUIA_MODELOS_PREGUNTA_GERARD.md)** con ejemplos detallados
        """)
